...
def main():
    ...
    if len(sys.argv) < 2:
        print("RAGFlow Knowledge Search")
        print("\nSimple usage (for AI):")
        print("  python search.py \"<query>\"")
        print("  python search.py \"<query>\" \"<dataset_id>\"")
        print("\nAdvanced usage:")
        print("  python search.py [OPTIONS] \"<search query>\"")
        print("\nOptions:")
    ...
    # Simple usage: python search.py "query" ["dataset_id"]
    if len(sys.argv) == 2 and not sys.argv[1].startswith('--'):
        # Just query provided
        query = sys.argv[1]
        dataset_ids = None
        search(env, query, dataset_ids)
        return

    if len(sys.argv) == 3 and not sys.argv[1].startswith('--') and not sys.argv[2].startswith('--'):
        # Query and dataset_id provided
        query = sys.argv[1]
